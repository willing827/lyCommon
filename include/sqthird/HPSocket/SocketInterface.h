...
public:
	...
	*/
	...
	virtual BOOL SetUserExtra(CONNID dwConnID, PVOID pExtra) = 0;
	virtual BOOL GetUserExtra(CONNID dwConnID, PVOID* ppExtra) = 0;
	...
	/* 获取某个连接的远程地址信息 */
	virtual BOOL GetRemoteAddress		(CONNID dwConnID, CHAR lpszAddress[], int& iAddressLen, USHORT& usPort)	= 0;
	/* 获取最近一次失败操作的错误代码 */
...
public:
	...
	*			FALSE	-- 失败，可通过 GetLastError() 获取错误代码
	*/
	virtual BOOL Start	(LPCSTR pszBindAddress, USHORT usPort)								= 0;
...
public:
	...
	*			FALSE	-- 失败，可通过 GetLastError() 获取错误代码
	*/
	virtual BOOL Start	(LPCSTR pszBindAddress = nullptr, BOOL bAsyncConnect = TRUE)						= 0;
...
public:
	...
	/* 获取某个连接的本地地址信息 */
	virtual BOOL GetLocalAddress(CONNID dwConnID, CHAR lpszAddress[], int& iAddressLen, USHORT& usPort)	= 0;
};
...
public:
	...
	*			FALSE	-- 失败，可通过 GetLastError() 获取错误代码
	*/
	virtual BOOL Start	(LPCSTR pszRemoteAddress, USHORT usPort, BOOL bAsyncConnect = FALSE)	= 0;
...
public:
	...
	virtual BOOL		GetLocalAddress	(CHAR lpszAddress[], int& iAddressLen, USHORT& usPort)	= 0;
	/* 获取 Remote Socket 的地址信息 */
	virtual BOOL		GetRemoteAddress(CHAR lpszAddress[], int& iAddressLen, USHORT& usPort) = 0;
	/* 获取连接中未发出数据的长度 */
