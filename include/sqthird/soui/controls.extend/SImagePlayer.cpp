...
{
	ISkinObj *pSkin = SSkinPoolMgr::getSingleton().GetSkin(strValue, 100);
...
BOOL SImagePlayer::_PlayFile( LPCTSTR pszFileName, BOOL bGif )
{
    ...
    ISkinObj *pSkin=pBuiltinSkinPool->GetSkin(key, 100);
    if(pSkin)
    {
    ...
    }else
    {
		...
		SOUI::SkinKey temp;
		temp.strName = key;
		temp.scale = pSkin->GetScale();
		pBuiltinSkinPool->AddKeyObject(temp, pGifSkin);//将创建的skin交给skinpool管理
        m_aniSkin = pGifSkin;
    }
