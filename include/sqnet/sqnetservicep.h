#ifndef SNQU_NETSERVICEP_H
#define SNQU_NETSERVICEP_H
...
class NetServiceP : public CTcpPullServerListener, public NetServiceWrapper
{
public:
	NetServiceP(){}
		...
		if (rel)
		{
				...
				m_server_engine->GetState() == SS_STARTED)
			{
				m_task_mgr_ptr->set_timer_task(port, HeartbeatDetectionSeconds, 
                    std::bind(&NetServiceP<session>::keepalive_timer_proc, this, std::placeholders::_1));
				m_site_id = site_id;
			}
		}
...
private:
	...
	void on_query_internal_key(CONNID connid)
	{
		NetServiceWrapper::query_internal_key(connid, 
			std::bind(&NetServiceP<session>::save_session_key, this, std::tr1::placeholders::_1, std::tr1::placeholders::_2), 
            std::bind(&NetServiceP<session>::fire_connected, this, std::tr1::placeholders::_1));
	}
    ...
    virtual EnHandleResult OnReceive(CONNID dwConnID, int iLength) override
    {
		...
		rel = NetServiceWrapper::receive_packet(dwConnID, iLength,
				std::bind(&NetServiceP<session>::on_heart_beat, this, std::tr1::placeholders::_1),
				std::bind(&NetServiceP<session>::on_query_internal_key, this, std::tr1::placeholders::_1),
				std::bind(&NetServiceP<session>::decrypt_packet, this, std::tr1::placeholders::_1, std::tr1::placeholders::_2, std::tr1::placeholders::_3, std::tr1::placeholders::_4),
				std::bind(&NetServiceP<session>::add_user_packet, this, std::tr1::placeholders::_1, std::tr1::placeholders::_2, std::tr1::placeholders::_3),
				std::bind(&NetServiceP<session>::get_message_factory, this, std::tr1::placeholders::_1));
    ...
    virtual EnHandleResult OnAccept(CONNID dwConnID, SOCKET soClient) override
    {
        ...
        if (linker_ptr)
        {
			...
			linker_ptr->bind_send_func(std::bind(&NetServiceP<session>::send_data, 
