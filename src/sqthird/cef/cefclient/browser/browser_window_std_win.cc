...
void BrowserWindowStdWin::CreateBrowser(
	...
	if (!CefCurrentlyOn(TID_UI)) {
		...
		CefPostTask(TID_UI, base::Bind(&BrowserWindowStdWin::CreateBrowser, this,
			parent_handle, rect, settings, request_context));
		return;
	}
...
void BrowserWindowStdWin::ShowPopup(ClientWindowHandle parent_handle,
  ...
  if (hwnd) {
    ...
    SetWindowPos(hwnd, NULL, x, y, static_cast<int>(width),
                 static_cast<int>(height), SWP_NOOWNERZORDER);
