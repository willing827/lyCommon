...
bool ClientHandler::OnProcessMessageReceived(
  ...
  if (delegate_)
	  delegate_->OnProcessMessageReceived(message);
...
#ifdef DEBUG
  FILE* file = NULL;
  int err = fopen_s(&file, console_log_file_.c_str(), "a");
  if (!err) {
	  std::stringstream ss;
...
void ClientHandler::OnTakeFocus(CefRefPtr<CefBrowser> browser, bool next) {
 ...
 bool ClientHandler::OnPreKeyEvent(CefRefPtr<CefBrowser> browser,
  ...
  //
  // µ÷ÊÔ¿ì½Ý¼ü
  if (cef3clt::GetContex()->is_dev() && IsKeyDown(VK_SHIFT))
  {
      if (event.windows_key_code == VK_F12)
      {
          ShowDevTools(browser, CefPoint());
      }
      else if (event.windows_key_code == VK_F11)
      {
...
bool ClientHandler::OnBeforePopup(
  ...
  CEF_REQUIRE_UI_THREAD();
  NotifyPopup(browser, frame, target_url);
  return true;
}
...
void ClientHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser) {
  ...
  if (!message_router_) {
    ...
    dialog::CreateMessageHandlers(message_handler_set_);
...
void ClientHandler::OnLoadEnd(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int httpStatusCode)
{
	...
	if (delegate_)
		delegate_->OnLoadOver(browser, frame, httpStatusCode, L"", L"");
}
...
void ClientHandler::OnLoadError(CefRefPtr<CefBrowser> browser,
  ...
  if (delegate_)
	  delegate_->OnLoadOver(browser, frame, errorCode, errorText, failedUrl);
...
void ClientHandler::ShowSSLInformation(CefRefPtr<CefBrowser> browser) {
  ...
  MainContext::Get()->GetBrowserWindowManager()->CreateBrowser(false, is_osr(), CefRect(), GetDataURI(ss.str(), "text/html"));
}
...
void ClientHandler::NotifyTitle(const CefString& title)
{
	...
	if (delegate_)
		delegate_->OnSetTitle(title.ToString());
}

void ClientHandler::NotifyPopup(CefRefPtr<CefBrowser> browser,
	CefRefPtr<CefFrame> frame, const CefString& target_url)
{
	if (!CURRENTLY_ON_MAIN_THREAD()) {
		// Execute this method on the main thread.
		MAIN_POST_CLOSURE(base::Bind(&ClientHandler::NotifyPopup, this, browser, frame, target_url));
		return;
	}

	if (delegate_)
		delegate_->OnPopup(browser, frame, target_url);
}
