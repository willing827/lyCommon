...
OsrWindowWin::OsrWindowWin(Delegate* delegate,
    ...
    : delegate_(delegate),
      ...
      hdc_(NULL),
      hrc_(NULL),
      device_scale_factor_(cef3::GetDeviceScaleFactor()),
...
LRESULT CALLBACK OsrWindowWin::OsrWndProc(HWND hWnd,
  ...
  switch (message) {
...
#endif
    case WM_LBUTTONDOWN:
		if (::IsWindow(self->hwnd_parent_))
			PostMessage(self->hwnd_parent_, cef3clt::CEF_WM_LBUTTONDOWN, wParam, lParam);
    ...
    case WM_LBUTTONUP:
		if (::IsWindow(self->hwnd_parent_))
			PostMessage(self->hwnd_parent_, cef3clt::CEF_WM_LBUTTONUP, wParam, lParam);
		self->OnMouseEvent(message, wParam, lParam);
		break;
    case WM_RBUTTONUP:
    case WM_MBUTTONUP:
    case WM_MOUSEMOVE:
		if (::IsWindow(self->hwnd_parent_))
			SendMessage(self->hwnd_parent_, cef3clt::CEF_WM_MOUSEMOVE, wParam, lParam);
		self->OnMouseEvent(message, wParam, lParam);
		break;
    case WM_MOUSELEAVE:
    case WM_MOUSEWHEEL:
  ...
  switch (message) {
    ...
    case WM_MOUSEMOVE: {
      ...
      if (mouse_rotation_) {
        ...
        renderer_.IncrementSpin(float(current_mouse_pos_.x - last_mouse_pos_.x), float(current_mouse_pos_.y - last_mouse_pos_.y));
