...
void ClientAppBrowser::OnBeforeCommandLineProcessing(
  ...
  if (process_type.empty()) {
	...
	//加载自定义flash插件
// 	command_line->AppendSwitchWithValue("ppapi-flash-version", "32.0.0.142");
// 	command_line->AppendSwitchWithValue("ppapi-flash-path", "C:\\Windows\\SysWOW64\\Macromed\\Flash\\pepflashplayer32_32_0_0_142.dll");

	command_line->AppendSwitch("ignore-certificate-errors");
	command_line->AppendSwitch("no-proxy-server");
	command_line->AppendSwitch("process-per-site");
	command_line->AppendSwitchWithValue("renderer-process-limit", "2");
	command_line->AppendSwitchWithValue("browser-subprocess-path", "CefRender.exe");
	//command_line->AppendSwitch(switches::kMultiThreadedMessageLoop);
