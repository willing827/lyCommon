...
RakNetGUID& RakNetGUID::operator = (const RakNetGUID& input)
{
	g = input.g;
...
RakNetGUID::RakNetGUID(uint64_t _g) { g = _g; systemIndex = (SystemIndex)-1; }
...
AddressOrGUID::AddressOrGUID(const SystemAddress& input)
{
...
AddressOrGUID::AddressOrGUID(const RakNetGUID& input)
{
	rakNetGuid = input;
...
AddressOrGUID& AddressOrGUID::operator = (const SystemAddress& input)
{
	rakNetGuid = UNASSIGNED_RAKNET_GUID;
	systemAddress = input;
	return *this;
...
AddressOrGUID& AddressOrGUID::operator = (const RakNetGUID& input)
{
	rakNetGuid = input;
	systemAddress = UNASSIGNED_SYSTEM_ADDRESS;
	return *this;
