...
bool FileList::AddData(const char *databuf, int32_t length, const char *uniqueId)
{
	if (!databuf || length <= 0)
		return false;
	...
	FileListNode n;
	n.context.op = 0;
	n.context.flnc_extraData1 = 0;
	n.context.flnc_extraData2 = 0;
	n.dataLengthBytes = length;
	...
	n.data=(char*) rakMalloc_Ex( (size_t) n.dataLengthBytes, _FILE_AND_LINE_ );
	RakAssert(n.data);
	memcpy(n.data, databuf, n.dataLengthBytes);
	n.fileLengthBytes = (unsigned) n.dataLengthBytes;

	n.dataLengthBytes=length;
	n.fileLengthBytes=length;
	n.isAReference=false;
	
	FileListNodeContext context(0, 0, 0, 0);
	n.context = context;
	if (n.context.dataPtr==0)
		n.context.dataPtr=n.data;
	if (n.context.dataLength==0)
		n.context.dataLength=length;
	
	n.filename=uniqueId;
	n.fullPathToFile="none";
